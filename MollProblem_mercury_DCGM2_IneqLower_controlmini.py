...
def loss(model, a_interior, z_interior, a_NBC, z_NBC, a_SC_upper, z_SC_upper, a_SC_lower, z_SC_lower):
    ...
    c_orig = tf.where(V_a <= 0, tf.zeros_like(V), u_deriv_inv(V_a))

    index1 = tf.cast(c_orig < z_interior+ r* a_interior, tf.float32) * tf.cast( (a_interior - X_low[0]) <=1e-10, tf.float32)
    index2 = tf.cast(c_orig >= z_interior+ r* a_interior, tf.float32) * tf.cast( (a_interior - X_low[0]) <=1e-10, tf.float32)
    
    c = c_orig * index1 + (z_interior+ r* a_interior) * index2 + c_orig * (1-index1) * (1-index2)
    ...
    c_SC_lower_a = u_deriv_inv(fitted_boundary_SC_lower_a)
    
    saving_SC_lower_a = z_SC_lower+r*a_SC_lower - c_SC_lower_a
    
    L3_lower_saving = tf.reduce_mean(tf.square(saving_SC_lower_a))
    ...
    L3 = L3_lower + L3_upper + L3_lower_saving
...
def control_c(V):
    ...
    index1 = tf.cast(c_orig < z_interior_tnsr+ r* a_interior_tnsr, tf.float32) * tf.cast( (a_interior_tnsr - X_low[0]) <=1e-10, tf.float32)
    index2 = tf.cast(c_orig >= z_interior_tnsr+ r* a_interior_tnsr, tf.float32) * tf.cast( (a_interior_tnsr - X_low[0]) <=1e-10, tf.float32)
    
    c = c_orig * index1 + (z_interior_tnsr+ r* a_interior_tnsr) * index2 + c_orig * (1-index1) * (1-index2)

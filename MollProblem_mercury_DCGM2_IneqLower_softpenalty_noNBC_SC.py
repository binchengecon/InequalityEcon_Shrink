...
# Save options
saveOutput = False
savefolder = 'Moll_softpenalty_Nosc/num_layers_FFNN_{}_activation_FFNN_{}_num_layers_RNN_{}_nodes_per_layer_{}/sampling_stages_{}_steps_per_sample_{}_starting_learning_rate_{}_penalty_{}_weight_{}/nSim_interior_{}_nSim_boundary_{}/id_{}/'.format(num_layers_FFNN, activation_FFNN, num_layers_RNN, nodes_per_layer, sampling_stages, steps_per_sample, starting_learning_rate, penalty, weight, nSim_interior, nSim_boundary, idd)
...
#    t_interior = np.random.uniform(low=t_low - t_oversample, high=T, size=[nSim_interior, 1])
    ...
    a_interior = np.random.uniform(low=0, high=1, size=[nSim_interior, 1])**2 * (X_high[0]-(X_low[0] - 0.02)) + X_low[0] - 0.02
...
def loss(model, a_interior, z_interior, a_NBC, z_NBC, a_SC_upper, z_SC_upper, a_SC_lower, z_SC_lower):
    ...
    index = tf.cast(a_interior <= X_low[0], tf.float32)
    
    u_c = u(c) - penalty * index * (a_interior - X_low[0])**2

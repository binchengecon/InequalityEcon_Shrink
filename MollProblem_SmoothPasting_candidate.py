...
def loss_neumann_boundary_alower(model, X):
    ...
    V_az_diff_zcd = V_az_zcd - u_deriv2(z_zcd+r*a_zcd)
    V_az_diff_zcd  = tf.reduce_mean(V_az_diff)
    
    
    V_az = tf.gradients(V_a, z)[0]

    V_az_diff = V_az - u_deriv2(z+r*a)
    
    return L, V_az_diff_zcd, V_az, u_deriv2(z+r*a), V_az_diff

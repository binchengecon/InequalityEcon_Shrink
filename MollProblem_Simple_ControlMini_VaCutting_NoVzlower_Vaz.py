...
def sampler(nSim_interior, nSim_boundary):
    ...
    z_zboundary = X_high[1] * np.ones((nSim_boundary, 1))
...
def loss(model, X_interior, X_alower, X_zboundary):
    ...
    L3 = loss_zboundarys(model, X_zboundary) + loss_concave(model, X_interior) + loss_neumann_boundary_alowerzlower(model, X_alower) 
    # L3 = loss_concave(model, X_interior)

    L4 = loss_neumann_boundary_alower(model, X_alower)

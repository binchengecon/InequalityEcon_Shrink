...
def sampler(nSim_interior, nSim_boundary):
    ...
    a_lower = X_low[0] * np.ones((nSim_interior, 1))
    z_lower = np.random.uniform(low=0, high=1, size=[nSim_interior, 1])**2 * (X_high[1]-X_low[1]) + X_low[1]


    return a_interior, z_interior, a_lower, z_lower
...
def loss(model, a_interior, z_interior, a_lower, z_lower):
    ...
    L3 = tf.reduce_mean(tf.zeros_like(diff_G_lower)) 
...
L1_tnsr, L2_tnsr, L3_tnsr = loss(
    model, a_interior_tnsr, z_interior_tnsr, a_lower_tnsr, z_lower_tnsr)
...
for i in range(sampling_stages):
    ...
    a_interior, z_interior, a_lower, z_lower = sampler(nSim_interior, nSim_boundary)
    
    # for a given sample, take the required number of SGD steps
    for _ in range(steps_per_sample):
        loss,L1,L2,L3,_ = sess.run([loss_tnsr, L1_tnsr, L2_tnsr, L3_tnsr, optimizer],
                                   feed_dict={a_interior_tnsr: a_interior, z_interior_tnsr: z_interior, a_lower_tnsr: a_lower, z_lower_tnsr: z_lower})

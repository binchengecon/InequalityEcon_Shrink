...
savefolder = 'baseline_findX_lowerlearning/Interal=[{},{}]/Slopeid_{}/learning_{}/backup_{}/'.format(args.lowerslope, args.upperslope, idd, starting_learning_rate, backup)
saveName   = 'Sannikov'
saveFigure = False
figureName = saveName
#%% Analytical Solution
...
def loss(model, X_interior, X_boundary):
    ...
    diff_V = -r * V + r*(a-c) + r * V_x * (X_interior-u_c+h_a) + V_xx/2*r**2*sigma**2*gamma_a**2
...
learning_rate = tf.train.exponential_decay(starting_learning_rate, global_step,1000, 0.99, staircase=True)

...
print("slope={}, xi={}".format(guess, xi))
# Save options
saveOutput = False

savefolder = 'baseline_ODEEmbedded4_robust/num_layers_FFNN_{}_activation_FFNN_{}_num_layers_RNN_{}_nodes_per_layer_{}/sampling_stages_{}_steps_per_sample_{}_learningrate_{}/nSim_interior_{}_nSim_boundary_{}/slope={}_xi_{}/'.format(num_layers_FFNN, activation_FFNN, num_layers_RNN, nodes_per_layer, sampling_stages, steps_per_sample, starting_learning_rate, nSim_interior, nSim_boundary, guess, xi)
...
ODE_drift = r*(X_plot-ODE_c**(1/2)+ODE_a**2/2+2*ODE_a/5 + (ODE_a+2/5)*sigma*ODE_h)
...
#    t_interior = np.random.uniform(low=t_low - t_oversample, high=T, size=[nSim_interior, 1])
    ...
    # X_interior = np.random.uniform(low=0, high=1, size=[nSim_interior, 1]) **(1/2) *(X_high - X_low) + X_low
...
def loss(model, X_interior, X_boundary, X_far):
    ...
    hh =  -1/xi*G_x*(a+2/5)*sigma
    
    gamma_a = gamma(a)
    
    
    Upper = G - a + c - G_x * (X_interior - u_c +h_a)
    Upper += -xi/2*hh**2 -G_x*(a+2/5)*sigma*hh
...
for i in range(sampling_stages):
    ...
    if i%4000==0:
        ...
        B_W = r*(X_plot-fitted_c**(1/2)+fitted_a**2/2+2*fitted_a/5  + (fitted_a+2/5)*sigma*fitted_h)
        fitted_drift = B_W
        ...
        axs["right dis"].plot(X_plot, fitted_h, color = 'red', label = 'NN Solution')
        axs["right dis"].plot(X_plot, ODE_h, color = 'blue', linestyle='--',label='ODE Solution')
        ...
        axs["left top"].plot(X_plot, np.log10(abs(fitted_V-ODE_F.reshape(-1,1))), color = 'red')
        axs["left top"].set_title("Difference in Profit $F(W)$")
        axs["left top"].set_xlim(X_low,X_high)
        axs["left top"].grid(linestyle=':')

        axs["left mid"].plot(X_plot, np.log10(abs(fitted_a-ODE_a.reshape(-1,1))), color = 'red')
        axs["left mid"].set_title("Difference in Effort $\\alpha(W)$")
        axs["left mid"].grid(linestyle=':')
        axs["left mid"].set_xlim(X_low,X_high)

        axs["right top"].plot(X_plot, np.log10(abs(fitted_c-ODE_c.reshape(-1,1))), color = 'red')
        axs["right top"].set_title("Difference in Consumption $\\pi(W)$")
        axs["right top"].grid(linestyle=':')
        axs["right top"].set_xlim(X_low,X_high)

        axs["right mid"].plot(X_plot, np.log10(abs(fitted_drift-ODE_drift.reshape(-1,1))), color = 'red')
        axs["right mid"].set_title("Difference in Drift of $W$")
        axs["right mid"].grid(linestyle=':')
        axs["right mid"].set_xlim(X_low,X_high)

        axs["down"].plot(X_plot, np.log10(abs(fitted_h-ODE_h.reshape(-1,1))), color = 'red')
        axs["down"].set_title("Difference in $h(W)$")
...
# plt.savefig(figureName + '_All.png')
plt.savefig('./Figure/' +savefolder+ '/' + figureName + '_All.png')
...
# plt.savefig(figureName + '_All.png')
plt.savefig('./Figure/' +savefolder+ '/' + figureName + '_Diff.png')
...
Fitted_matrix[:,9:10] = ODE_h.reshape(-1,1)
...
Fitted_matrix[:,14:15] = np.log10(abs(fitted_h-ODE_h.reshape(-1,1)))

...
parser.add_argument("--lowerpoint", type=float)
parser.add_argument("--upperpoint", type=float)
parser.add_argument("--id_point", type=int)
...
lowerpoint = args.lowerpoint
upperpoint = args.upperpoint
num_point = args.num_point
id_point = args.id_point

Z = np.linspace(lowerpoint, upperpoint, num_point)
...
# Save options
saveOutput = False
savefolder = 'Moll_SP2/num_layers_FFNN_{}_activation_FFNN_{}_num_layers_RNN_{}_nodes_per_layer_{}/sampling_stages_{}_steps_per_sample_{}_starting_learning_rate_{}_weight_{}/nSim_interior_{}_nSim_boundary_{}/[{},{}]_{}/id_point_{}/id_{}/'.format(num_layers_FFNN, activation_FFNN, num_layers_RNN, nodes_per_layer, sampling_stages, steps_per_sample, starting_learning_rate, weight, nSim_interior, nSim_boundary, lowerpoint, upperpoint, num_point, id_point, idd)
...
def sampler(nSim_interior, nSim_boundary):
    ...
    z_alower05 = z_cd * np.ones((1,1))
    z_alower1 = np.random.uniform(low=0, high=1, size=[int(nSim_interior/2)-1, 1]) * ( z_cd - X_low[1]) + X_low[1]
    z_alower2 = np.random.uniform(low=0, high=1, size=[int(nSim_interior/2)-1, 1]) * (X_high[1] - z_cd) + z_cd
    z_alower = np.concatenate([z_alower0, z_alower05, z_alower1,z_alower2],axis=0)
...
def value_info(model, X):
    ...
    return V, V_a, V_az, V_aa, V_z, V_zz
...
def loss_neumann_boundary_alower(model, X):
    ...
    index_left = tf.cast( z <= z_cd , tf.float32)
    index_right = tf.cast( z > z_cd , tf.float32)

    Diff_left =  V_a-u_deriv(z+r*a)
    Diff_right = tf.minimum(tf.zeros_like(V),V_a-u_deriv(z+r*a))


    Diff = Diff_left * index_left + Diff_right * index_right

    L = tf.reduce_mean(tf.square(Diff))


    a_zcd = tf.ones((1,1)) * X_low[0]
    z_zcd = tf.ones((1,1)) * z_cd
    V_zcd = model(tf.stack([a_zcd[:,0],z_zcd[:,0]], axis=1))
    V_a_zcd = tf.gradients(V_zcd, a_zcd)[0]
    V_az_zcd = tf.gradients(V_a_zcd, z_zcd)[0]

    V_az_diff = V_az_zcd - u_deriv2(z_zcd+r*a_zcd)

    V_az_diff = tf.reduce_mean(V_az_diff)
...
def loss(model, X_interior, X_alower, X_zboundary):
    ...
    L4, V_az_diff = loss_neumann_boundary_alower(model, X_alower)

    return L1, L2, L3, L4, Loss_V_interior, Loss_V_alower, V_az_diff
...
# loss 
L1_tnsr, L2_tnsr, L3_tnsr, L4_tnsr, Loss_V_interior_tnsr, Loss_V_alower_tnsr, V_az_diff_tnsr = loss(
    model, X_interior_tnsr, X_alower_tnsr, X_zboundary_tnsr)
Loss_tnsr = L1_tnsr + L2_tnsr +  L3_tnsr + weight * L4_tnsr

c_tnsr, s_tnsr = control_cs(model, X_interior_tnsr)
V_tnsr, V_a_tnsr, V_az_tnsr, V_aa_tnsr, V_z_tnsr, V_zz_tnsr = value_info(model,X_interior_tnsr)

# set optimizer - NOTE THIS IS DIFFERENT FROM OTHER APPLICATIONS!
global_step = tf.Variable(0, trainable=False)
learning_rate = tf.train.exponential_decay(starting_learning_rate, global_step,40000, 0.95, staircase=True)
...
for i in range(sampling_stages):
    ...
    #     Loss_V_interior = sess.run([Loss_V_interior_tnsr],
    #                                feed_dict={X_interior_tnsr: X_interior, X_alower_tnsr: X_alower, X_zboundary_tnsr: X_zboundary})[0]
    ...
    #     test_lossL1_list.append(averageL1)
    #     test_lossL2_list.append(averageL2)
    ...
    if i%4000==0:
        ...
        fitted_V, fitted_Va, fitted_Vaz, fitted_Vaa, fitted_Vz, fitted_Vzz = sess.run([V_tnsr, V_a_tnsr, V_az_tnsr, V_aa_tnsr, V_z_tnsr, V_zz_tnsr], feed_dict={X_interior_tnsr: X_interior})
        fitted_c, fitted_s = sess.run([c_tnsr, s_tnsr], feed_dict={X_interior_tnsr: X_interior})
        
        Loss_V_interior, Loss_V_alower, V_az_diff = sess.run([Loss_V_interior_tnsr, Loss_V_alower_tnsr, V_az_diff_tnsr],
                                   feed_dict={X_interior_tnsr: X_interior, X_alower_tnsr: X_alower, X_zboundary_tnsr: X_zboundary})
        ...
        fig = plt.figure(figsize=(16, 9))
        ax = fig.add_subplot(111, projection='3d')
        ax.plot_surface(A, Z, fitted_s.reshape(n_plot, n_plot), cmap='viridis')
        ax.view_init(35, 35)
        ax.set_xlabel('$a$')
        ax.set_ylabel('$z$')
        # ax.set_zlabel('$v(a,z)$')
        ...
        plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_Save_{}.png'.format(i),bbox_inches='tight')
        ...
        fig = plt.figure(figsize=(16, 9))
        plt.plot(Z[:, 0], fitted_Vaz.reshape(n_plot, n_plot)[:, 0],
                label='Value', color='black')
        plt.plot(Z[:, 0], u_deriv2(Z[:, 0]+r*A[:, 0]),
                label='BaseLine', color='red')
        plt.axvline(x=z_cd, color='blue',linestyle='-')
        plt.xlabel('$z$')
        plt.xlim(0.8,1.25)
        plt.legend()
        plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_VazSlice_{}.png'.format(i),bbox_inches='tight')
        ...
        # ax.semilogy(range(len(test_lossL1_list)), test_lossL1_list, 'r-', label='L1')
        # ax.semilogy(range(len(test_lossL2_list)), test_lossL2_list, 'b-', label='L2')
        # ax.set_xlabel('$n_{epoch}$')
        # ax.set_ylabel('$\\phi^{n_{epoch}}$')
        # ax.legend()
        # # if saveFigure:
        # plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_TestLossL12_{}.png' .format(i),bbox_inches='tight')
...
plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_Save.png',bbox_inches='tight')
...
plt.xlim(0.8,1.25)
plt.legend()
plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_VazSlice.png',bbox_inches='tight')
...
# ax.set_ylabel('$\\phi^{n_{epoch}}$')
# ax.legend()
# plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_TestLossL12.png',bbox_inches='tight')

# fig = plt.figure(figsize=(16,9))
# ax = fig.add_subplot(111)
# ax.semilogy(range(len(test_lossL2_list)), test_lossL2_list, 'k-')
# ax.set_xlabel('$n_{epoch}$')
# ax.set_ylabel('$\\phi^{n_{epoch}}$')
# # if saveFigure:
# plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_TestLossL2.png',bbox_inches='tight')

...
# Save options
saveOutput = False
savefolder = 'Moll_KFE_pseudo/num_layers_FFNN_{}_activation_FFNN_{}_num_layers_RNN_{}_nodes_per_layer_{}/sampling_stages_{}_steps_per_sample_{}_starting_learning_rate_{}/nSim_interior_{}_nSim_boundary_{}/id_{}/'.format(num_layers_FFNN, activation_FFNN, num_layers_RNN, nodes_per_layer, sampling_stages, steps_per_sample, starting_learning_rate, nSim_interior, nSim_boundary, idd)
...
def loss_KFE(model, modelg, a_interior, z_interior, a_NBC, z_NBC, a_SC_upper, z_SC_upper, a_SC_lower, z_SC_lower):
    ''' Compute total loss for training.
    ...
    g = modelg(tf.stack([a_interior[:,0], z_interior[:,0]], axis=1))
    ...
    positive_g = tf.minimum(g, tf.zeros_like(V))

    L1_KFE = tf.reduce_mean(tf.square(KFE)) + tf.reduce_mean(tf.square(positive_g))
    
    # L2_KFE = tf.square((X_high[0]-X_low[0])*(X_high[1]-X_low[1])*tf.reduce_mean(g) - 1)
    L2_KFE = tf.reduce_mean(tf.zeros_like(g))

    # L3_KFE = tf.square(tf.reduce_mean(g*a_interior))
    L3_KFE = tf.reduce_mean(tf.zeros_like(g))


    return L1_KFE, L2_KFE, L3_KFE
    # return L1, L2
...
modelg = DGM2.DCGM2Net(X_low, X_high, nodes_per_layer, num_layers_FFNN,num_layers_RNN, 2, activation_FFNN)
...
# loss 
L1_KFE_tnsr, L2_KFE_tnsr, L3_KFE_tnsr = loss_KFE(
    model, modelg, a_interior_tnsr, z_interior_tnsr, a_NBC_tnsr, z_NBC_tnsr,a_SC_upper_tnsr,z_SC_upper_tnsr,a_SC_lower_tnsr,z_SC_lower_tnsr)
loss_KFE_tnsr = L1_KFE_tnsr + L2_KFE_tnsr + L3_KFE_tnsr

# value function

V = model(tf.stack([a_interior_tnsr[:, 0], z_interior_tnsr[:, 0]], axis=1))
g = modelg(tf.stack([a_interior_tnsr[:, 0], z_interior_tnsr[:, 0]], axis=1))
...
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss_tnsr, var_list = model.trainable_variables)
optimizer_KFE = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss_KFE_tnsr, var_list = modelg.trainable_variables)
...
for i in range(sampling_stages):
    ...
    for _ in range(steps_per_sample):
        ...
        print("{}, {:.8f}, {:.8f}, {:.8f}, {:.8f}".format(i, loss, L1, L2, L3))
    ...
    if i%2000==0:
        ...
        plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_Value_{}.pdf'.format(i),bbox_inches='tight')
        ...
        # ax.set_title('Deep Learning Solution')
        plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_Va_{}.pdf'.format(i),bbox_inches='tight')
        ...
        # ax.set_title('Deep Learning Solution')
        plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_VaSlice_{}.pdf'.format(i),bbox_inches='tight')
...
for i in range(sampling_stages):
    ...
    # for a given sample, take the required number of SGD steps
    for _ in range(steps_per_sample):
        loss_KFE,L1_KFE,L2_KFE,L3_KFE,_ = sess.run([loss_KFE_tnsr, L1_KFE_tnsr, L2_KFE_tnsr, L3_KFE_tnsr, optimizer_KFE],
                                   feed_dict={a_interior_tnsr: a_interior, z_interior_tnsr: z_interior, a_NBC_tnsr: a_NBC, z_NBC_tnsr: z_NBC, a_SC_upper_tnsr: a_SC_upper, z_SC_upper_tnsr: z_SC_upper, a_SC_lower_tnsr: a_SC_lower, z_SC_lower_tnsr: z_SC_lower})
        loss_KFE_list.append(loss_KFE)
    
        # print(loss, L1, L2, L3, L1_KFE,L2_KFE,L3_KFE, i)
        print("{}, {:.8f}, {:.8f}, {:.8f}, {:.8f}".format(i, loss_KFE, L1_KFE, L2_KFE, L3_KFE))
    ...
    if i%2000==0:
        ...
        fitted_g = sess.run([g], feed_dict={a_interior_tnsr: Xgrid[:,0:1], z_interior_tnsr: Xgrid[:,1:2]})[0]
        fitted_g = fitted_g.reshape(n_plot, n_plot)
        fitted_g = fitted_g/np.sum(fitted_g)
        
        fig = plt.figure(figsize=(16, 9))
        ax = fig.add_subplot(111, projection='3d')
        ax.plot_surface(A, Z, fitted_g, cmap='viridis')
        # ax.view_init(35, 35)
        ax.set_xlabel('$a$')
        ax.set_ylabel('$z$')
        # ax.set_zlabel('$v(a,z)$')
        ...
        plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_g_{}.pdf'.format(i),bbox_inches='tight')
...
plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_g.pdf',bbox_inches='tight')
...
fig = plt.figure(figsize=(16,9))
ax = fig.add_subplot(111)
ax.semilogy(range(len(loss_list)), loss_KFE_list, 'k-')
ax.set_xlabel('$n_{epoch}$')
ax.set_ylabel('$\\phi^{n_{epoch}}$')
# if saveFigure:
plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_Loss_KFE.pdf',bbox_inches='tight')

...
def loss_differentialoperator_alower(model, X):
    ...
    VariationalIneq = tf.minimum(diff_V,VarDiff)
    # L = tf.reduce_mean(tf.square(diff_V))
    L = tf.reduce_mean(tf.square(VariationalIneq))
    
    
    
    return diff_V, L, VarDiff
...
def loss(model, X_interior, X_alower, X_zboundary):
    ...
    Loss_V_alower, L2, VarDiff_alower = loss_differentialoperator_alower(model, X_alower)
    
    L3 = loss_zboundarys(model, X_zboundary) + loss_concave(model, X_interior)

    
    return L1, L2, L3, Loss_V_interior, Loss_V_alower, VarDiff_alower
...
# loss 
L1_tnsr, L2_tnsr, L3_tnsr, Loss_V_interior_tnsr, Loss_V_alower_tnsr, VarDiff_alower_tnsr = loss(
...
for i in range(sampling_stages):
    ...
    if i%4000==0:
        ...
        Loss_V_interior, Loss_V_alower, VarDiff_alower = sess.run([Loss_V_interior_tnsr, Loss_V_alower_tnsr, VarDiff_alower_tnsr],
                                   feed_dict={X_interior_tnsr: X_interior, X_alower_tnsr: X_alower, X_zboundary_tnsr: X_zboundary})
        ...
        plt.plot(Z_alower[:, 0], VarDiff_alower[:, 0],
        label='VarDiff', color='red')        
        
        plt.xlabel('$z$')
        plt.title('Loss: L1Average={}, L2Average={}'.format(average_alower_L1, average_alower_L2))
        
        plt.legend()
        plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_LossV_VarDiff_Slice_{}.png'.format(i),bbox_inches='tight')
...
plt.legend()
plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_LossV_VarDiff_Slice.png',bbox_inches='tight')

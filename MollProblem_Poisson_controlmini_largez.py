...
def sampler(nSim_interior, nSim_boundary):
    ...
    a_alower = X_low[0] * np.ones((nSim_boundary, 1))
...
def loss(model, X_interior, X_alower):
...
model = DGM2.DCGM4Net(X_low, X_high, nodes_per_layer, num_layers_FFNN,num_layers_RNN, 1, 2, activation_FFNN)

# tensor placeholders (_tnsr suffix indicates tensors)
# inputs (time, space domain interior, space domain at initial time)
# t_interior_tnsr = tf.placeholder(tf.float32, [None,1])
X_interior_tnsr = tf.placeholder(tf.float32, [None,1])
X_alower_tnsr = tf.placeholder(tf.float32, [None,1])
# X_zboundary_tnsr = tf.placeholder(tf.float32, [None,1])


# loss 
L1_tnsr, L2_tnsr, L3_tnsr, L4_tnsr, Loss_V_interior_tnsr, Loss_V_alower_tnsr = loss(
    model, X_interior_tnsr, X_alower_tnsr)
Loss_tnsr = L1_tnsr + L2_tnsr +  L3_tnsr + L4_tnsr

c_tnsr = control_c(model, X_interior_tnsr)
V_tnsr, V_a_tnsr, S_tnsr = value_info(model,X_interior_tnsr)
...
for i in range(sampling_stages):
    ...
    for _ in range(steps_per_sample):
        loss,L1,L2,L3,L4,_ = sess.run([Loss_tnsr, L1_tnsr, L2_tnsr, L3_tnsr, L4_tnsr, optimizer],
                                   feed_dict={X_interior_tnsr: X_interior, X_alower_tnsr: X_alower})
    ...
    if i%4000==0:
        ...
        fitted_V, fitted_Va, fitted_s = sess.run([V_tnsr, V_a_tnsr, S_tnsr], feed_dict={X_interior_tnsr: X_interior})
        ...
        averageL1 = np.mean(np.abs(Loss_V_interior).reshape(n_plot, 2))
        averageL2 = np.mean(np.square(Loss_V_interior).reshape(n_plot, 2))

        fig = plt.figure(figsize=(16, 9))
        plt.plot(X_interior[:,0], fitted_V.reshape(n_plot,2))        
        # plt.plot(X_interior[:,0], fitted_V.reshape(n_plot,2)[:,1])        
        plt.xlabel('$a$')
        # plt.title('Loss: L1Average={}, L2Average={}'.format(average_alower_L1, average_alower_L2))
        
        plt.legend()
        plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_V_{}.png'.format(i),bbox_inches='tight')

        fig = plt.figure(figsize=(16, 9))
        plt.plot(X_interior[:,0], fitted_Va.reshape(n_plot,2))        
        # plt.plot(X_interior[:,0], fitted_Va.reshape(n_plot,2)[:,1])        
        plt.xlabel('$a$')
        # plt.title('Loss: L1Average={}, L2Average={}'.format(average_alower_L1, average_alower_L2))
        ...
        fig = plt.figure(figsize=(16, 9))
        plt.plot(X_interior[:,0], fitted_s.reshape(n_plot,2)[:,0],label='z={}'.format(z[0]))        
        plt.plot(X_interior[:,0], fitted_s.reshape(n_plot,2)[:,1],label='z={}'.format(z[1]))        
        plt.xlabel('$a$')
        # plt.title('Loss: L1Average={}, L2Average={}'.format(average_alower_L1, average_alower_L2))
        
        plt.legend()
        plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_S_{}.png'.format(i),bbox_inches='tight')

...
def loss_neumann_boundary_alower(model, X):
    ...
    index0 = tf.cast(V_azz<0,tf.float32)
    index1 = 1-index0
    
    Diff0 = V_a-u_deriv(z+r*a)
    Diff1 = tf.minimum(V_a-u_deriv(z+r*a),tf.zeros_like(V))

    Diff = Diff0 * index0  + Diff1 * index1
    
    L = tf.reduce_mean(tf.square(Diff))

    return L

def loss_neumann_boundary_alowerzlower(model, X):
    a = X[0:1,0:1]
    z = X[0:1,1:2]
    ...
    Diff_a = V_a-u_deriv(z+r*a)
    Diff_az = V_az-u_deriv2(z+r*a)

    L = tf.reduce_mean(tf.square(Diff_a)) + tf.reduce_mean(tf.square(Diff_az))
...
def loss(model, X_interior, X_alower, X_zboundary):
    ...
    L4 = loss_neumann_boundary_alower(model, X_alower) + loss_neumann_boundary_alowerzlower(model, X_alower)

...
def sampler(nSim_interior, nSim_boundary):
    ...
    z_alower = np.random.uniform(low=X_low[1], high=X_high[1], size=[nSim_interior, 1])
...
def loss_differentialoperator_alower(model, X):
    ...
    # L = tf.reduce_mean(tf.square(diff_V))
    L = 10*tf.reduce_max(tf.abs(diff_V)) + tf.reduce_mean(tf.square(diff_V))
    return diff_V, L

...
def grid_sampler(nSim_interior):
    ''' Sample time-space points from the function's domain; points are sampled
        uniformly on the interior of the domain, at the initial/terminal time points
        and along the spatial boundary at different time points. 
    ...
    aspace = np.linspace(-0.02, 4, nSim_interior)
    zspace = np.linspace(zmean*0.8, zmean*1.2, nSim_interior)
...
def loss_differentialoperator_alower(model, X):
    ...
    diff_V = -rho*V+u_c_new+V_a * \
        (z+r*a-c_new)+(-the*tf.math.log(z)+sig2/2)*z*V_z + sig2*z**2/2*V_zz
...
X_interior_test, X_alower_test, X_zboundary_test = grid_sampler(n_test)
...
for i in range(sampling_stages):
    ...
    if i%10==0:
        ...
        loss_validation = sess.run([Loss_tnsr],
                                   feed_dict={X_interior_tnsr: X_interior_test, X_alower_tnsr: X_alower_test, X_zboundary_tnsr: X_zboundary_test})[0]
        loss_validation_list.append(loss_validation)


    if i%4000==0:
        
        X_interior, X_alower, X_zboundary = grid_sampler(n_plot)

...
# Save options
saveOutput = False
savefolder = 'gamma=2,IneqLower2_num_layers_FFNN_{}_num_layers_RNN_{}_nodes_per_layer_{}_sampling_stages_{}_steps_per_sample_{}_nSim_interior_{}_nSim_boundary_{}'.format(num_layers_FFNN, num_layers_RNN, nodes_per_layer, sampling_stages, steps_per_sample, nSim_interior, nSim_boundary)
...
def u_u_deriv_inv(c):
    return c**((gamma-1)/gamma)/(1-gamma)
...
def loss(model, a_interior, z_interior, a_NBC, z_NBC, a_SC_upper, z_SC_upper, a_SC_lower, z_SC_lower):
    ...
    c = tf.where(V_a <= 0, tf.ones((nSim_interior,1)), u_deriv_inv(V_a))

    u_c = tf.where(V_a <= 0, tf.ones((nSim_interior,1)), u_u_deriv_inv(V_a))
...
model = DGM2.DCGM2Net(X_low, X_high, nodes_per_layer, num_layers_FFNN,num_layers_RNN, 2)
...
def control_c(V):
    ...
    c = tf.where(V_a <= 0, tf.ones(tf.shape(V)), u_deriv_inv(V_a))
...
plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_Consumption.png')
...
plt.savefig('./Figure/' +savefolder+ '/' + saveName + '_Vaa.png')
...
# if saveFigure:
plt.savefig( './Figure/' +savefolder+ '/' + saveName + '_Loss.png')

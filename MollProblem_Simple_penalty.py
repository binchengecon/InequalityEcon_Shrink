...
def sampler(nSim_interior, nSim_boundary):
    ...
    X_interior = np.random.uniform(low=X_low-X_oversample, high=X_high, size=[nSim_interior, 2])
...
def loss_differentialoperator(model, X):
    ...
    c = u_deriv_inv(V_a)
    index  = tf.cast(a<=X_low[0], tf.float32)

    u_c = u(c) 
    u_c += -penalty*index*(a-X_low[0])**2

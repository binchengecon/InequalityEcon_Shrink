...
#    t_interior = np.random.uniform(low=t_low - t_oversample, high=T, size=[nSim_interior, 1])
    ...
    a_interior = np.random.uniform(low=0, high=1, size=[nSim_interior, 1]) * (X_high[0]-X_low[0]) + X_low[0]
    # z_interior = np.random.uniform(low=X_low[1], high=X_high[1], size=[nSim_interior, 1])
    z_interior = np.random.uniform(low=0, high=1, size=[nSim_interior, 1]) * (X_high[1]-X_low[1]) + X_low[1]
...
#    X_interior = np.random.uniform(low=X_low * X_multiplier_low, high=X_high * X_multiplier_high, size=[nSim_interior, 1])
    ...
    z_SC_lower = np.random.uniform(
        low=0, high=1, size=[nSim_boundary, 1]) * (X_high[1]-X_low[1]) + X_low[1]
...
def loss(modelV, modelg, a_interior, z_interior, a_NBC, z_NBC, a_SC_upper, z_SC_upper, a_SC_lower, z_SC_lower):
    ...
    V = modelV(tf.stack([a_interior[:,0], z_interior[:,0]], axis=1))
    ...
    smallerthan1_g = tf.maximum(g-1, tf.zeros_like(g))
    ...
    fitted_boundary_NBC = modelV(tf.stack([a_NBC[:,0], z_NBC[:,0]], axis=1))
    ...
    fitted_boundary_SC_lower = modelV(
    ...
    fitted_boundary_SC_upper = modelV(
    ...
    return L1, L2, L3, L1_KFE, L2_KFE, L3_KFE 
    # return L1, L2
...
modelV = DGM3.DCGM3Net(X_low, X_high, nodes_per_layer, num_layers_FFNN,num_layers_RNN, 2, 1, activation_FFNN)
modelg = DGM3.DCGM3Net(X_low, X_high, nodes_per_layer, num_layers_FFNN,num_layers_RNN, 2, 1, activation_FFNN)
...
# loss 
L1_tnsr, L2_tnsr, L3_tnsr, L1_KFE_tnsr, L2_KFE_tnsr, L3_KFE_tnsr = loss(
    modelV, modelg, a_interior_tnsr, z_interior_tnsr, a_NBC_tnsr, z_NBC_tnsr,a_SC_upper_tnsr,z_SC_upper_tnsr,a_SC_lower_tnsr,z_SC_lower_tnsr)
loss_tnsr = L1_tnsr + L2_tnsr + L3_tnsr + L1_KFE_tnsr + L2_KFE_tnsr + L3_KFE_tnsr

# value function

V = modelV(tf.stack([a_interior_tnsr[:, 0], z_interior_tnsr[:, 0]], axis=1))
...
for i in range(sampling_stages):
    ...
    for _ in range(steps_per_sample):
        loss,L1,L2,L3,L1_KFE,L2_KFE,L3_KFE,_ = sess.run([loss_tnsr, L1_tnsr, L2_tnsr, L3_tnsr, L1_KFE_tnsr, L2_KFE_tnsr, L3_KFE_tnsr, optimizer],
        ...
        print("{}, {:.8f}, {:.8f}, {:.8f}, {:.8f}, {:.8f}, {:.8f}, {:.8f}".format(i, loss, L1, L2, L3, L1_KFE,L2_KFE,L3_KFE))
